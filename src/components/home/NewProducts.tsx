    ...
    <div className="bg-white rounded-[28px] flex-shrink-0 flex flex-col pt-3 pb-5 px-3 gap-7 w-[280px] md:w-[340px] xl:w-[420px]">
      ...
      <div className="relative rounded-[20px] bg-[#f0f0f0] overflow-hidden flex items-start justify-between p-5 group h-[220px] md:h-[270px] xl:h-[320px]">
        ...
        <div className="relative z-10 flex items-start justify-between w-full">
          <div className="bg-[#1a7a4a] rounded-full px-3 py-2 text-white text-xs font-extrabold tracking-wider">
            LANÇAMENTO
          </div>
          <button
...
export function NewProducts() {
  ...
  const newProducts = useMemo(() => {
    ...
    return apiProducts
      .filter((product: any) => product.isNew === true)
      .sort((a: any, b: any) => (a.newOrder ?? 0) - (b.newOrder ?? 0))
      .slice(0, 8)
      .map((product: any) => ({
        id: product.id,
  ...
  if (!mounted || isLoading || !newProducts || newProducts.length === 0) {
    return null
  }

  return (
    <section className="w-full bg-[#fff5ec] flex flex-col pt-0 pb-[64px]">
      {/* Header */}
          ...
          <div className="flex gap-3 mt-8">
            <button
              ...
              className={`rounded-lg w-14 h-14 flex items-center justify-center transition ${
                canScrollLeft
                  ? 'bg-[#0d3d24] hover:bg-[#072918] cursor-pointer'
